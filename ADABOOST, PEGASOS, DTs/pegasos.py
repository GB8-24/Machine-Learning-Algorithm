...
def objective_function(X, y, w, lamb):
    ...
    sum = 0
    for i in range(0,N):
        h_x = y[i] * np.dot(w.T, X[i])
        if (1-h_x)<0:
            sum+=0
        else:
            sum+=1-h_x

    min = np.min(np.linalg.norm(w))*(lamb/2)
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        for i in A_t:
            w_t = w_t + np.reshape((eta_t / k) * ytrain[i] * Xtrain[i], (D, 1))

        val = (1 / (lamb ** 0.5)) / np.linalg.norm(w_t)
        if val > 1:
            min=1
        else:
            min = val
      ...
      #  w_t_plus1 = min(1,(1/(lamb**0.5))/np.linalg.norm(w_t_half))*w_t_half
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    #n,features = np.asarray(Xtest).shape

    #pred = w.T*Xtest
    #acc = 0

    #for iter in range (0,Xtest.shape):
        #if pred[iter] >= t:
        #    pred[iter]=1
       # else:
      #      pred[iter]=-1
     ...
     #   if pred[iter] == ytest[iter]:
    #         acc=acc+1;

   # test_acc = acc/ytest.shape

    Xtest = np.asarray(Xtest)
    n, features = Xtest.shape
    y_pred = np.zeros(n)
    count = 0
    Xtest_array = np.asarray(Xtest)
    for i in range(n):
        if np.dot(Xtest_array[i], w) < t:
            y_pred[i] = -1
        else:
            y_pred[i] = 1
        if y_pred[i] == ytest[i]:
