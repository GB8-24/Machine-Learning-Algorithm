...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for iteration in range(max_iterations):
        wold = w
        for i in range(N):
            score = np.matmul(X[i], w) + b
            pred = sigmoid(score)
            err = pred - y[i]
            grad = X[i] * err

            w -= (step_size / N) * grad.T
            b -= (step_size / N) * err


        if np.all(np.absolute(w-wold)<np.exp(-7)):
            #converged
            return w,b
...
def binary_predict(X, w, b):
    ...
    for i in range(0,N):
        score = np.matmul(X[i], w) + b
        sigmoid_value = sigmoid(score)
        preds[i] = 1 if sigmoid_value >= 0.5 else 0
...
def multinomial_train(X, y, C, 
    ...
    TODO: add your code here
    """
    y_k_hot = np.zeros((N, C))
    for i in range(N):
        y_k_hot[i][y[i]] = 1

    for iteration in range(max_iterations):
        for i in range(N):
            score = np.matmul(X[i], w.T) + b
            softmax_value = softmax(score)

            for k in range(C):
                error = softmax_value[k] - y_k_hot[i][k]
                gradient = X[i] * error
                w[k] -= step_size * gradient
                b[k] -= step_size * error
...
def multinomial_predict(X, w, b):
    ...
    for i in range(N):
        scores = np.zeros(C)
        for k in range(C):
            scores[k] = np.matmul(X[i],w[k].T) + b[k]

        preds[i] = np.argmax(softmax(scores))
    assert preds.shape == (N,)
    return preds
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(C):
        ...
        w[i], b[i] = binary_train(X, y_copy, w0, b0, step_size, 1000)
...
def OVR_predict(X, w, b):
    ...
    """
    for i in range(0,N):
        maximum = np.zeros((C, 1))
        for j in range(C):
            w_T = w[j].T
            score = np.matmul(X[i], w_T) + b[j]
            maximum[j] = sigmoid(score)
        preds[i] = np.argmax(max)
