...
def transform_image(image, code_vectors):
    ...
    nb_clusters = code_vectors.shape[0]
    N = image.shape[0]
    M = image.shape[1]
    dist = np.zeros((N*M,nb_clusters))
    data = image.reshape(N * M, 3)
    for k in range(nb_clusters):
        t=(data - code_vectors[k])**2
        dist[:,k] = t.sum(1)
    # membership vector
    R = np.argmin(dist,axis=1)
    # Convert to one-hot encoding
    r = np.zeros((R.size,nb_clusters))
    r[np.arange(R.size),R] = 1
    return r.dot(code_vectors).reshape(N,M,3)
