...
def transform_image(image, code_vectors):
    ...
    N, M = image_copy.shape[:2]
    data = image_copy.reshape(N * M, 3)

    dist = np.zeros(len(code_vectors))
    test = 0

    for i in range(0,len(data)):
        for j in range(0,len(code_vectors)):
            dist[j] = np.linalg.norm(data[i]-code_vectors[j])
        test = np.argmin(dist)
        data[i]=code_vectors[test]
